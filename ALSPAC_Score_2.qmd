...
dx_vars_2 <- c('AN_noex', 'bn_present', 'binge_criteria_met', 'purge_disorder_present', 'AAN_noex', 'sub_restrict_noex', 'sub_bn_present', 'binge_sub_criteria_met', 'sub_restrict_exonly')
  ...
  mutate(
    ...
    ed_dx_noex = (
      case_when(
        ...
        sub_bn_present == 1 ~ 6,
        binge_sub_criteria_met == 1 ~ 7,
        sub_restrict_noex == 1 ~ 8,
        sub_restrict_exonly == 1 ~ 9,
...
dx_data <- dx_data |> 
  mutate(
    ...
    aan_noex = ifelse(as.integer(ed_dx_noex) == 5, 1, ifelse(is.na(ed_dx_noex), NA, 0)),
    an = ifelse(as.integer(ed_dx) == 1, 1, ifelse(is.na(ed_dx), NA, 0)),
    an_noex = ifelse(as.integer(ed_dx_noex) == 1, 1, ifelse(is.na(ed_dx_noex), NA, 0)),
    sub_restrict = ifelse(as.integer(ed_dx) == 8, 1, ifelse(is.na(ed_dx), NA, 0)),
    sub_restrict_noex = ifelse(as.integer(ed_dx_noex) == 8, 1, ifelse(is.na(ed_dx_noex), NA, 0)),
    sub_restrict_exonly = ifelse(as.integer(ed_dx_noex) == 9, 1, ifelse(is.na(ed_dx_noex), NA, 0)),
    other_ed = ifelse(as.integer(ed_dx) %in% other_ed_codes, 1, ifelse(is.na(ed_dx), NA, 0)),
    other_ed_noex = ifelse(as.integer(ed_dx_noex) %in% other_ed_codes, 1, ifelse(is.na(ed_dx_noex), NA, 0)),
...
dx_data <- dx_data |> 
  mutate(
    ...
    ed_restrict_dx_noex = case_when (
      ...
      aan_noex == 1 ~ 2, 
      other_ed_noex == 1 ~ 3,
      sub_restrict_noex == 1 ~ 4, 
      sub_restrict_exonly == 1 ~ 5,
      no_ed == 1 ~ 0
    )
  )
