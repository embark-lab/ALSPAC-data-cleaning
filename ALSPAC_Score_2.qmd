...
dx_data <- dx_data |> 
  mutate(
    ...
    sub_restrict = ifelse(as.integer(ed_dx) == 9, 1, ifelse(is.na(ed_dx), NA, 0)),
    sub_restrict_noex = ifelse(as.integer(ed_dx_noex) == 9, 1, ifelse(is.na(ed_dx_noex), NA, 0)),
    sub_restrict_exonly = ifelse(as.integer(ed_dx_noex) == 10, 1, ifelse(is.na(ed_dx_noex), NA, 0)),
    restrict_missingwt_noex_2 = ifelse(as.integer(ed_dx_noex) == 8, 1, ifelse(is.na(ed_dx_noex), NA, 0)),
...
dx_data <- dx_data |> 
  mutate(
    ...
    ed_restrict_dx_noex = case_when (
      ...
      aan_noex == 1 ~ 2, 
      other_ed_noex == 1 ~ 3,
      restrict_missingwt_noex_2 == 1 ~ 4,
      sub_restrict_noex == 1 ~ 5, 
      sub_restrict_exonly == 1 ~ 6,
      no_ed == 1 ~ 0
    )
  )
