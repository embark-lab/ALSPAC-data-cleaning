...
### 5. Add mean BMIz scores for ages 7:12; 9:14; 11:16

```{r}
# Define a list of age pairs (first age, second age)
age_pairs <- list(c(7, 12), c(9, 14), c(11, 16))
...
mbmiz_data <- BMIZ_7_12 %>%
  left_join(BMIZ_9_14, by = "id") %>%
  left_join(BMIZ_11_16, by = "id") 
```
...
BMI_bestavail_wide <- full_join(BMI_bestavail_wide, mbmiz_data) |> 
  mutate(bmiz_drop_raw.168 = mbmiz_7_12 - `bmiz_bestavail.168`, 
         bmiz_drop.168 = case_when(bmiz_drop_raw.168 > .5 ~ 1,
                                   bmiz_drop_raw.168 < .5 ~ 0 ),
         bmiz_drop_raw.192 = mbmiz_9_14 - `bmiz_bestavail.192`, 
         bmiz_drop.192 = case_when(bmiz_drop_raw.192 > .5 ~ 1,
                                   bmiz_drop_raw.192 < .5 ~ 0 ),
         bmiz_drop_raw.216 = mbmiz_11_16 - `bmiz_bestavail.216`, 
...
static_vars <- c('id', 'sex', 'birth_order', 'occupation_class_mum', 'occupation_class_partner', 'education_mum', 'education_mum_d', 'education_partner', 'education_partner_d', 'ethnicity_child', 'parent_highest_occ.1', 'parent_highest_occ.2', 'parent_highest_occupation', 'ethnicity_mum', 'ethnicity_ptnr')
cols <- cols[!cols %in% static_vars]

wide_data <- long_data |> 
  tidyr::pivot_wider(
    id_cols = c(id, sex, birth_order, occupation_class_mum, occupation_class_partner, education_mum, education_mum_d, education_partner, education_partner_d, ethnicity_child, parent_highest_occ.1, parent_highest_occ.2, parent_highest_occupation, ethnicity_mum, ethnicity_ptnr),
    values_from = cols,
